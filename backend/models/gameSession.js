...
async function createQuestions(round, categories) {
    ...
    for (const [key, value] of Object.entries(round)) {
        for (i = 0; i < value; i++) {
            category = categories.length > 0 ? "&category=" + categories[Math.floor(Math.random() * categories.length)] : "";
            await fetch(`https://opentdb.com/api.php?amount=1${category}&difficulty=${key}`)
