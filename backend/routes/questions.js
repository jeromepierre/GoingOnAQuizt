...
router.post(
  ...
  asyncMiddleware.asyncMiddleware(async (req, res, next) => {
    ...
    round = {
      categories:
        req.body.categories &&
        getStringArray.getStringArray(req.body.categories).length > 0
          ? getStringArray.getStringArray(req.body.categories)
          : categoriesIdLess.categoriesIdLess,
      difficulties:
        req.body.difficulties &&
        getStringArray.getStringArray(req.body.difficulties).length > 0
          ? getStringArray.getStringArray(req.body.difficulties)
          : ["hard", "medium", "easy"],
    ...
    for (const [key, value] of Object.entries(round.categories)) {
      categoriesNumber.push(
        categories.categories.find((x) => x.name === value).id
      );
    }
    let configuredRound = configureRound.configureRound(round);
    let createdQuestions = await createQuestions.createQuestions(
      configuredRound,
      categoriesNumber
    );
